...
fn integration_scorecard() {
  ...
  let raydium_cpi =
    read(&format!("{PROGRAM_SRC}/instructions/venues/raydium_amm.rs"));
  let swap_route = read("src/swap_route/mod.rs");
  let state = read(&format!("{PROGRAM_SRC}/state.rs"));
  let template_venue =
    read(&format!("{PROGRAM_SRC}/instructions/venues/template.rs"));
  ...
  let example_done = [
    ...
    swap_route.contains("PoolProtocol::RaydiumAMM")
      && swap_route.contains("Venue::RaydiumAmm"),
  ];
  ...
  let section =
    std::env::var("SCORECARD_SECTION").unwrap_or_else(|_| "both".into());
