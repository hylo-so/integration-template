...
async fn build_venue<V: SuiteVenue>(
  ...
  let mut venue = V::from_account(&pool, &account)
    .expect("failed to build venue from account");
...
async fn sim_quote_request(
  ...
  let (token_a, token_a_program) =
    (tokens[idx_0].pubkey, tokens[idx_0].get_token_program());
  let (token_b, token_b_program) =
    (tokens[idx_1].pubkey, tokens[idx_1].get_token_program());
  ...
  let mut account_a =
    Account::new(LAMPORTS_PER_SOL, TokenAccount::LEN, &token_a_program);
  let a = TokenAccount {
    mint: token_a,
  ...
  let mut account_b =
    Account::new(LAMPORTS_PER_SOL, TokenAccount::LEN, &token_b_program);
  let b = TokenAccount {
    mint: token_b,
    owner: keypair.pubkey(),
    state: AccountState::Initialized,
...
pub async fn construction<V: SuiteVenue>(config: &SuiteConfig) {
  ...
  for (in_idx, out_idx) in venue.directions_num() {
    ...
    for (edge, amount) in [("lower", lower), ("upper", upper)] {
      let q = assert_no_alloc(|| {
        venue.quote(exact_in(input_mint, output_mint, amount))
      })
