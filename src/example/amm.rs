...
pub fn load_amm_keys(
  ...
  let amm =
    raydium::state::AmmInfo::load_from_bytes(&account_data).map_err(|_| {
      TradingVenueError::DeserializationFailed(
        "Failed to load AmmInfo from bytes".into(),
      )
    })?;
  let data = AmmKeys {
    amm_pool: *amm_key,
    ...
    .map_err(|_| {
      TradingVenueError::AmmMethodError(
        "Failed to determine authority ID".into(),
      )
    })?,
...
pub fn calculate_pool_vault_amounts_from_accounts(
  ...
  let amm: raydium::state::AmmInfo =
    transmute_one_pedantic::<raydium::state::AmmInfo>(transmute_to_bytes(
      &amm_account
        .as_ref()
        .ok_or_else(|| {
          TradingVenueError::MissingState("AMM account missing".into())
        })?
        .clone()
        .data,
    ))
    .map_err(|_| {
      TradingVenueError::DeserializationFailed(
        "Failed to deserialize AMM Info".into(),
      )
    })?;
  let amm_pc_vault = spl_token::state::Account::unpack(
    &amm_pc_vault_account
      .as_ref()
      .ok_or_else(|| {
        TradingVenueError::MissingState("AMM PC vault account missing".into())
      })?
      .clone()
      .data,
  )
  .map_err(|_| {
    TradingVenueError::DeserializationFailed(
      "Failed to deserialize token account".into(),
    )
  })?;
  ...
  .map_err(|_| {
    TradingVenueError::DeserializationFailed(
      "Failed to deserialize AMM coin vault".into(),
    )
  })?;
  let (amm_pool_pc_vault_amount, amm_pool_coin_vault_amount) =
    if raydium::state::AmmStatus::from_u64(amm.status).orderbook_permission() {
      return Err(TradingVenueError::AmmMethodError(
        "Pool has orderbook permission, not able to swap with v2 instruction"
          .into(),
      ));
    } else {
      let (amm_pool_pc_vault_amount, amm_pool_coin_vault_amount) =
        ...
        .map_err(|_| {
          TradingVenueError::AmmMethodError(
            "Failed to calculate total without take PNL with no orderbook"
              .into(),
          )
        })?;
...
fn max_amount_with_slippage(input_amount: u64, slippage_bps: u64) -> u64 {
  ...
  u64::try_from(quotient)
    .expect("increasing input amount by slippage overflowed u64")
}
...
pub fn swap_with_slippage(
  ...
  if (swap_direction == SwapDirection::Coin2PC
    && other_amount_threshold >= pc_vault_amount)
    || (swap_direction == SwapDirection::PC2Coin
      && other_amount_threshold >= coin_vault_amount)
  {
    return Err(TradingVenueError::MathError(
...
pub fn swap_exact_amount(
  ...
  let other_amount_threshold = if swap_base_in {
    ...
    let swap_in_after_deduct_fee = U128::from(amount_specified)
      ...
      .ok_or(TradingVenueError::MathError(
        "swap_in_after_deduct_fee checked math error".into(),
      ))?;
    ...
    let swap_in_before_add_fee =
      raydium::math::Calculator::swap_token_amount_base_out(
...
pub fn swap_v2(
  ...
  let swap_instruction = if swap_base_in {
    ...
    .map_err(|_| {
      TradingVenueError::AmmMethodError("Failed to perform v2 swap".into())
    })?
  } else {
    unimplemented!()
  };
