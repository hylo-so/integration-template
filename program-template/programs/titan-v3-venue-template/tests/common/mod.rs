...
//!
//! `tests/hylo_route.rs` calls [`run_swap_route`]: quote off-chain, execute
//! `swap_route_v3` in LiteSVM, and assert the simulated output matches the
//! quote.
//!
...
async fn load_route_accounts(
  ...
  accounts_to_load
    .extend(swap_ix.accounts.iter().map(|account| account.pubkey));
...
async fn swap_route_attempt<V: RouteVenue>(
  ...
  let mut venue =
    V::from_account(&config.pool, &venue_account).expect("from_account failed");
  venue
    .update_state(&cache)
    .await
  ...
  for (input_index, output_index) in venue.directions_num() {
    ...
    for amount in sample_amounts(lower, upper) {
      ...
      let route_ix =
        build_route_instruction(payer.pubkey(), titan_pda, &venue, &request);
      let output_token = token_for_mint(&venue, request.output_mint);
      let output_ata = get_associated_token_address_with_program_id(
        &payer.pubkey(),
        &request.output_mint,
