...
use crate::instructions::venues::hylo_router;
...
impl SwapRouteV3<'_> {
  pub fn execute<'info>(
    ...
    for swap in swaps.iter() {
      ...
      let available_balance = input_token_balance
        .saturating_sub(reserved_balances[swap.from as usize]);
      ...
      if input_amount > 0 {
        let leg_accounts = &ctx.remaining_accounts[remaining_account_index
          ..remaining_account_index + account_increment];
    ...
    let titan_pda_output_token_account = ctx
      ...
      .ok_or(TemplateError::MissingRemainingAccount)?;
    let raw_output_balance =
      extract_token_amount(titan_pda_output_token_account)?;
...
fn read_token_ledger<'info>(
  ...
  let token_account_slice =
    data.get(8..40).ok_or(TemplateError::InvalidAccountData)?;
  ...
  let amount_slice =
    data.get(40..48).ok_or(TemplateError::InvalidAccountData)?;
...
fn close_token_account<'info>(
  ...
  match signer_seeds {
    ...
    None => {
      close_account(CpiContext::new(token_program.to_account_info(), accounts))
    }
  }
}
...
fn transfer_sol_and_sync<'info>(
  ...
  let sync_ix =
    spl_token::instruction::sync_native(token_program.key, wsol_ata.key)?;
